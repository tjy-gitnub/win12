    ...
    div.addEventListener(
      ...
      (event) => {
        if (div.getAttribute("appname") != undefined) {
          return showcm(event, "desktop.icon", [
            div.getAttribute("appname"),
            div.getAttribute("iconIndex"),
          ]);
        }
        return false;
      },
...
let cms = {
  "save-bar": [
    function (arg) {
      return [
        '<i class="bi bi-window-x"></i> 移除',
        `removeEdgeSaveUrl('${arg}')`,
      ];
    },
  ],
  titbar: [
  ...
  desktop: [
    ...
    [
      '<i class="bi bi-brush"></i> 个性化',
      `openapp('setting');$('#win-setting > div.menu > list > a.enable.appearance')[0].click()`,
    ],
  ],
  "desktop.icon": [
    function (arg) {
  ...
  smapp: [
    ...
    function (arg) {
      return [
        ...
        "var s=`<div class='b' ondblclick=openapp('" +
          arg[0] +
          ...
          "'><p>" +
          arg[1] +
          "</p></div>`;$('#desktop').append(s);desktopItem[desktopItem.length]=s;addMenu();saveDesktop();",
      ];
    },
    function (arg) {
      return [
  ...
  "explorer.folder": [
    ...
    (arg) => {
      return [
        '<i class="bi bi-arrow-up-right-square"></i> 在新标签页中打开',
        `apps.explorer.newtab('${arg}');`,
      ];
    },
    (arg) => {
    ...
    (arg) => {
      if ($("#win-explorer>.path>.tit>.path>div.text").length > 1)
        return [
          ...
          `apps.explorer.copy_or_cut('${arg}','copy')`,
        ];
      return "null";
    },
    (arg) => {
      if ($("#win-explorer>.path>.tit>.path>div.text").length > 1)
        return [
          ...
          `apps.explorer.copy_or_cut('${arg}','cut')`,
        ];
      return "null";
    },
    (arg) => {
      if ($("#win-explorer>.path>.tit>.path>div.text").length > 1)
        return [
          '<i class="bi bi-input-cursor-text"></i> 重命名',
          `apps.explorer.rename('${arg}')`,
        ];
      return "null";
    },
  ],
    ...
    (arg) => {
      if ($("#win-explorer>.path>.tit>.path>div.text").length > 1)
        return [
          ...
          `apps.explorer.add($('#win-explorer>.path>.tit')[0].dataset.path,'新建文本文档.txt')`,
        ];
      return "null";
    },
    (arg) => {
      if ($("#win-explorer>.path>.tit>.path>div.text").length > 1)
        return [
          ...
          `apps.explorer.add($('#win-explorer>.path>.tit')[0].dataset.path,'新建文件夹',type='files')`,
        ];
      return "null";
    },
    (arg) => {
      if ($("#win-explorer>.path>.tit>.path>div.text").length > 1)
        return [
          '<i class="bi bi-file-earmark-arrow-down"></i> 粘贴',
          `apps.explorer.paste($('#win-explorer>.path>.tit')[0].dataset.path,'新建文件夹',type='files')`,
        ];
      return "null";
    },
    (arg) => {
  ...
  "explorer.tab": [
    (arg) => {
      return [
        '<i class="bi bi-x"></i> 关闭标签页',
        `m_tab.close('explorer',${arg})`,
      ];
    },
  ],
  "edge.tab": [
  ...
  "taskmgr.processes": [
    (arg) => {
      return [
        '<i class="bi bi-x"></i> 结束任务',
        `apps.taskmgr.taskkill('${arg}')`,
      ];
    },
  ],
};
...
dps = {
  ...
  "notepad.view": [
    ...
    [
      ...
      "font_window=true;hidedp(true);showwin('notepad-fonts');apps.notepadFonts.reset();",
    ],
  ],
};
...
let nts = {
  ...
  feedback: {
    ...
    btn: [{ type: "main", text: "关闭", js: "closenotice();" }],
  },
  widgets: {
    cnt: `
            <p class="tit">添加小组件</p>
  ...
  ZeroDivision: {
    ...
    cnt: `<p class="tit">错误</p>
            <p>除数不得等于0</p>`,
    btn: [{ type: "main", text: "确定", js: "closenotice();" }],
  },
  "Can-not-open-file": {
    cnt:
      `<p class="tit">` +
    ...
    btn: [
      ...
      {
        type: "detail",
        text: "在 Micrsoft Edge 中搜索",
        js:
          "closenotice();openapp('edge');window.setTimeout(() => {apps.edge.newtab();apps.edge.goto(" +
          run_cmd +
          ");}, 300);",
      },
    ],
...
请按任意键继续.&nbsp;.&nbsp;.<input type="text" onkeydown="hidewin('terminal')"></input>`)
      ...
      } else if (cmds.includes("-s") || cmds.includes("/s")) {
        //关机
        if (
          (cmds.indexOf("-t") != -1 &&
            cmd.length /*判断是否-t后有其他参数*/ >=
              cmds.indexOf("-t") +
                2) /*先加一，获取当下标是从1开始的时候的下标索引；再加一，获取下一项。配合数组.length使用*/ ||
          (cmds.indexOf("/t") != -1 &&
            cmd.length /*判断是否-t后有其他参数*/ >= cmds.indexOf("/t") + 2)
        ) {
          str = "";
          if (cmds.includes("-t")) {
            str = "-t";
          ...
          if (
            !isNaN(
              cmds[cmds.indexOf(str) + 1] /*这里只加一是因为下标是从0开始的*/
            )
          ) {
            ...
            nts["shutdown"] = {
              cnt:
                `
                            ...
                            <p>Windows 将在 ` +
                calcTimeString(num) +
                ` 后关闭。</p>`, // 如果必须原生样式的，建议改为 num<60 ? 1 : Math.floor(num / 60) + ` 分钟后关闭。</p>`,
              btn: [{ type: "main", text: "关闭", js: "closenotice();" }],
            };
            shutdown_task[shutdown_task.length] = setTimeout(
        ...
        //重启
        if (
          (cmds.indexOf("-t") != -1 && cmd.length >= cmds.indexOf("-t") + 2) ||
          (cmds.indexOf("/t") != -1 && cmd.length >= cmds.indexOf("/t") + 2)
        ) {
          /*详见上面的注释*/
          str = "";
      ...
      } else if (cmds.includes("-a") || cmds.includes("/a")) {
        //取消电源操作
...
let apps = {
  setting: {
    init: () => {
      ...
      $(
        "#win-setting>.page>.cnt.update>.setting-list>div:last-child>.alr>a.checkbox"
      )[
    ...
    theme_get: () => {
      ...
      // 实时获取主题
      $.get(
        "https://api.github.com/repos/tjy-gitnub/win12-theme/contents"
      ).then((cs) => {
        cs.forEach((c) => {
          if (c.type == "dir") {
            $.get(c.url).then((cnt) => {
                ...
                if (cn.name == "theme.json") {
                  $.getJSON(
                    ...
                    if ($("#set-theme>loading").length)
                      $("#set-theme").html("");
                    $("#set-theme").append(
                      `<a class="a act" onclick="apps.setting.theme_set('${c.name}')" style="background-image:url('https://tjy-gitnub.github.io/win12-theme/${c.name}/view.jpg')">${c.name}</a>`
                    );
                  });
                }
              });
    ...
    checkUpdate: () => {
      $(
        "#win-setting>.page>.cnt.update>.lo>.update-main .notice"
      )[0].innerText = "正在检查更新...";
      $(
        "#win-setting>.page>.cnt.update>.lo>.update-main .detail"
      )[0].innerHTML = "&nbsp;";
      $("#win-setting>.page>.cnt.update>.setting-list>.update-now").addClass(
        "disabled"
      );
      $(
        "#win-setting>.page>.cnt.update>.setting-list>.update-now>div>p:first-child"
      )[0].innerText = "正在检查更新...";
      $(
        "#win-setting>.page>.cnt.update>.setting-list>.update-now>div>p:last-child"
      )[0].innerHTML = "&nbsp;";
      $(
        "#win-setting>.page>.cnt.update>.lo>.update-main>div:last-child"
      ).addClass("disabled");
      fetch("https://api.github.com/repos/tjy-gitnub/win12/commits").then(
        (res) => {
          res.json().then((json) => {
            ...
            if (sha != json[0].sha) {
              ...
              if (msg.match(/v[0-9]*\.[0-9]*\.[0-9]*/)) {
                ...
                window.setTimeout(() => {
                  $(
                    "#win-setting>.page>.cnt.update>.lo>.update-main .notice"
                  )[0].innerText = "Windows 12 有更新可用";
                  $(
                    "#win-setting>.page>.cnt.update>.lo>.update-main .detail"
                  )[0].innerText = `目前最新版本: ${msg}`;
                  $(
                    "#win-setting>.page>.cnt.update>.lo>.update-main>div:last-child"
                  ...
                  $(
                    "#win-setting>.page>.cnt.update>.setting-list>.update-now"
                  ).removeClass("disabled");
                }, 6000);
              } else {
                window.setTimeout(() => {
                  ...
                  $(
                    "#win-setting>.page>.cnt.update>.lo>.update-main .notice"
                  )[0].innerText = "Windows 12 目前是最新版本";
                  $(
                    "#win-setting>.page>.cnt.update>.lo>.update-main .detail"
                  )[0].innerText = `上次检查时间: ${da.getFullYear()}年${
                    da.getMonth() + 1
                  }月${da.getDate()}日，${da.getHours()}: ${da.getMinutes()}`;
                  $(
                    "#win-setting>.page>.cnt.update>.lo>.update-main>div:last-child"
                  ).removeClass("disabled");
                  $(
                ...
                btn: [
                  {
                    type: "main",
                    text: "确定",
                    js: "closenotice();showwin('run');$('#win-run>.open>input').select();",
                  },
                  {
                    type: "cancel",
                    text: "在 Micrsoft Edge 中搜索",
                    js:
                      "closenotice();openapp('edge');window.setTimeout(() => {apps.edge.newtab();apps.edge.goto('https://www.bing.com/search?q=" +
  ...
  taskmgr: {
    ...
    load: (init_all = true) => {
      if (init_all == true) {
        $(
          "#win-taskmgr>.main>.cnt.performance>.content>.performance-graph>.graph-cpu>.information>.left>div:nth-child(3)>.value"
        )[0].innerText = apps.taskmgr.tasks.length;

        apps.taskmgr.cpuCanvas = $(
          "#win-taskmgr>.main>.cnt.performance>.content>.performance-graph>.graph-cpu canvas.chart"
        )[0];
        apps.taskmgr.cpuBgCanvas = $(
          "#win-taskmgr>.main>.cnt.performance>.content>.performance-graph>.graph-cpu canvas.bg"
        )[0];
        apps.taskmgr.initgraph("cpu", "#2983cc");

        apps.taskmgr.memoryCanvas = $(
          "#win-taskmgr>.main>.cnt.performance>.content>.performance-graph>.graph-memory canvas.chart"
        )[0];
        apps.taskmgr.memoryBgCanvas = $(
          "#win-taskmgr>.main>.cnt.performance>.content>.performance-graph>.graph-memory canvas.bg"
        )[0];
        apps.taskmgr.initgraph("memory", "#660099");
        apps.taskmgr.memoryCanvas2 = $(
          "#win-taskmgr>.main>.cnt.performance>.content>.performance-graph>.graph-memory canvas.chart2"
        )[0];
        ...
        apps.taskmgr.diskCanvas = $(
          "#win-taskmgr>.main>.cnt.performance>.content>.performance-graph>.graph-disk canvas.chart"
        )[0];
        apps.taskmgr.diskBgCanvas = $(
          "#win-taskmgr>.main>.cnt.performance>.content>.performance-graph>.graph-disk canvas.bg"
        )[0];
        apps.taskmgr.initgraph("disk", "#008000");
        apps.taskmgr.diskCanvas2 = $(
          "#win-taskmgr>.main>.cnt.performance>.content>.performance-graph>.graph-disk canvas.chart2"
        )[0];
        apps.taskmgr.diskLastPos2 = {
          read: [
            apps.taskmgr.diskCanvas2.width,
            apps.taskmgr.diskCanvas2.height,
          ],
          write: [
            apps.taskmgr.diskCanvas2.width,
            apps.taskmgr.diskCanvas2.height,
        ...
        apps.taskmgr.diskBgCanvas2 = $(
          "#win-taskmgr>.main>.cnt.performance>.content>.performance-graph>.graph-disk canvas.bg2"
        )[0];
        ...
        for (var i = 0; i < 4; i++) {
          apps.taskmgr.gpuCanvas[i] = $(
            "#win-taskmgr>.main>.cnt.performance>.content>.performance-graph>.graph-gpu>.graphs>.graph" +
              (i + 1) +
              ">.chart>canvas.chart"
          )[0];
          apps.taskmgr.gpuBgCanvas[i] = $(
            "#win-taskmgr>.main>.cnt.performance>.content>.performance-graph>.graph-gpu>.graphs>.graph" +
              (i + 1) +
              ">.chart>canvas.bg"
          )[0];
          apps.taskmgr.gpuCtx[i] = apps.taskmgr.gpuCanvas[i].getContext("2d");
          apps.taskmgr.gpuBgCtx[i] =
            apps.taskmgr.gpuBgCanvas[i].getContext("2d");
          apps.taskmgr.gpuCtx[i].strokeStyle = "#2983cc";
          apps.taskmgr.gpuCtx[i].fillStyle = "#2983cc22";
          apps.taskmgr.gpuCtx[i].lineWidth = 4;
          apps.taskmgr.gpuLastPos[i] = [
            apps.taskmgr.gpuCanvas[i].width,
            apps.taskmgr.gpuCanvas[i].height,
          ];
        ...
        for (var i = 0; i < 2; i++) {
          apps.taskmgr.gpuCanvas2[i] = $(
            "#win-taskmgr>.main>.cnt.performance>.content>.performance-graph>.graph-gpu>.gpu2-" +
              (i + 1) +
              ">canvas.chart2"
          )[0];
          apps.taskmgr.gpuBgCanvas2[i] = $(
            "#win-taskmgr>.main>.cnt.performance>.content>.performance-graph>.graph-gpu>.gpu2-" +
              (i + 1) +
              ">canvas.bg2"
          )[0];
          apps.taskmgr.gpuCtx2[i] = apps.taskmgr.gpuCanvas2[i].getContext("2d");
          apps.taskmgr.gpuBgCtx2[i] =
            apps.taskmgr.gpuBgCanvas2[i].getContext("2d");
          apps.taskmgr.gpuCtx2[i].strokeStyle = "#2983cc";
          apps.taskmgr.gpuCtx2[i].fillStyle = "#2983cc22";
          apps.taskmgr.gpuCtx2[i].lineWidth = 3;
          apps.taskmgr.gpuLastPos2[i] = [
            apps.taskmgr.gpuCanvas2[i].width,
            apps.taskmgr.gpuCanvas2[i].height,
          ];
        ...
        apps.taskmgr.gpuCanvas3 = $(
          "#win-taskmgr>.main>.cnt.performance>.content>.performance-graph>.graph-gpu>.graphs>canvas.usage"
        )[0];
        ...
        apps.taskmgr.gpuLastPos3 = [
          apps.taskmgr.gpuCanvas3.width,
          apps.taskmgr.gpuCanvas3.height,
        ];

        apps.taskmgr.wifiCanvas = $(
          "#win-taskmgr>.main>.cnt.performance>.content>.performance-graph>.graph-wifi canvas.chart"
        )[0];
        apps.taskmgr.wifiBgCanvas = $(
          "#win-taskmgr>.main>.cnt.performance>.content>.performance-graph>.graph-wifi canvas.bg"
        )[0];
    ...
    graph: (name) => {
      ...
      $(
        "#win-taskmgr>.main>.cnt.performance>.content>.select-menu>." + name
      ).addClass("check");
    },
    generateProcesses: () => {
      let processList = [];
      ...
      let cpusum = 0,
        ...
        diskUsing =
          Number(Math.random()) >
          0.7; /*, color = window.getComputedStyle(page, null).getPropertyValue('--href')*/
      var Search_length = 0;
      for (const elt of apps.taskmgr.tasks) {
        let cpu = Number((Math.random() * max).toFixed(1)),
          memory =
            apps.taskmgr.memory != 0
              ? apps.taskmgr.memory / apps.taskmgr.tasks.length +
                Number(((Math.random() - 0.5) / 5).toFixed(1))
              : Number((Math.random() * max).toFixed(1)),
          disk =
            Number((Math.random() * max).toFixed(1)) > max / 1.2 && diskUsing
              ? max * Number(Math.random().toFixed(1))
              : 0;
        cpusum = Number((cpusum + cpu).toFixed(1));
        memorysum = Number((memorysum + memory).toFixed(1));
        disksum = Number((disksum + disk).toFixed(1));
        if (
          document.getElementById("tsk-search").value != "" &&
          document.getElementById("tsk-search").style.display == "" &&
          !elt.name
            .toLowerCase()
            .includes(
              document
                .getElementById("tsk-search")
                .value.toLowerCase() /* 搜索时转换成小写 */
            )
        ) {
          continue; //搜索
        }
    ...
    loadProcesses: (processList = apps.taskmgr.processList) => {
      const processContainer = $(
        "#win-taskmgr>.main>.cnt.processes tbody.view"
      )[0];
      ...
      if (
        $(
          "#win-taskmgr>.main>.cnt.processes tbody.view>tr.select>td:first-child>.text"
        )[0]
      ) {
        selected = $(
          "#win-taskmgr>.main>.cnt.processes tbody.view>tr.select>td:first-child>.text"
      ...
      for (const elt of processList) {
        ...
        newElt.innerHTML = `<td><div class="text"><div class="icon" style="background-image: url('${
          elt.icon ? elt.icon : ""
        }');"></div>${
          elt.name
        }</div></td><td style="text-align: right;background-color: ${
          page.classList.contains("dark") ? "#193662" : "#7ec6ec"
        }${elt.cpu >= max / 1.3 ? "ff" : "aa"};">${elt.cpu.toFixed(
          1
        )}%</td><td style="text-align: right;background-color: ${
          page.classList.contains("dark") ? "#193662" : "#7ec6ec"
        }${elt.memory >= max / 1.3 ? "ff" : "aa"};">${elt.memory.toFixed(
          1
        )}%</td><td style="text-align: right;background-color: ${
          page.classList.contains("dark") ? "#193662" : "#7ec6ec"
        }${elt.disk >= max / 1.3 ? "ff" : "aa"};">${elt.disk.toFixed(
          1
        )}%</td><td>${
          ["非常低", "非常低", "非常低", "低", "中"][
    ...
    sort: (
      processList = apps.taskmgr.processList,
      type = apps.taskmgr.sortType,
      order = apps.taskmgr.sortOrder
    ) => {
    ...
    performanceLoad: () => {
      $(
        "#win-taskmgr>.main>.cnt.performance>.content>.performance-graph>.graph-cpu>.information>.left>div:nth-child(1)>.value"
      )[0].innerText = `${apps.taskmgr.cpu.toFixed(1)}%`;
      $(
        "#win-taskmgr>.main>.cnt.performance>.content>.performance-graph>.graph-cpu>.information>.left>div:nth-child(2)>.value"
      )[0].innerText = `${((3200 / 100) * apps.taskmgr.cpu).toFixed(1)} GHz`;
      $(
        "#win-taskmgr>.main>.cnt.performance>.content>.performance-graph>.graph-cpu>.information>.left>div:nth-child(4)>.value"
      )[0].innerText = `${
        apps.taskmgr.tasks.length * (5 + Math.floor(Math.random() * 5))
      }`;
      $(
        "#win-taskmgr>.main>.cnt.performance>.content>.performance-graph>.graph-cpu>.information>.left>div:nth-child(5)>.value"
      )[0].innerText = `${
        apps.taskmgr.tasks.length * (10 + Math.floor(Math.random() * 20))
      }`;
      ...
      hour %= 24;
      $(
        "#win-taskmgr>.main>.cnt.performance>.content>.performance-graph>.graph-cpu>.information>.left>div:nth-child(6)>.value"
      )[0].innerText = `${day}:${String(hour).length < 2 ? "0" : ""}${hour}:${
        String(min).length < 2 ? "0" : ""
      }${min}:${String(sec).length < 2 ? "0" : ""}${sec}`;
      $(
        "#win-taskmgr>.main>.cnt.performance>.content>.select-menu>.graph-cpu>.right>.data>.value1"
      )[0].innerText = `${apps.taskmgr.cpu.toFixed(1)}%`;
      $(
        "#win-taskmgr>.main>.cnt.performance>.content>.select-menu>.graph-cpu>.right>.data>.value2"
      )[0].innerText = `${((3200 / 100) * apps.taskmgr.cpu).toFixed(1)}GHz`;

      $(
        "#win-taskmgr>.main>.cnt.performance>.content>.performance-graph>.graph-memory>.information>.left>div:nth-child(1)>.value"
      )[0].innerText = `${apps.taskmgr.memory.toFixed(1)} GB (0 MB)`;
      $(
        "#win-taskmgr>.main>.cnt.performance>.content>.performance-graph>.graph-memory>.information>.left>div:nth-child(2)>.value"
      )[0].innerText = `${(100 - apps.taskmgr.memory).toFixed(1)} GB`;
      $(
        "#win-taskmgr>.main>.cnt.performance>.content>.performance-graph>.graph-memory>.information>.left>div:nth-child(3)>.value"
      )[0].innerText = `${(apps.taskmgr.memory - 2).toFixed(1)} / 100 GB`;
      $(
        "#win-taskmgr>.main>.cnt.performance>.content>.performance-graph>.graph-memory>.information>.left>div:nth-child(4)>.value"
      )[0].innerText = `${(apps.taskmgr.memory / 2).toFixed(2)} GB`;
      $(
        "#win-taskmgr>.main>.cnt.performance>.content>.select-menu>.graph-memory>.right>.data>.value1"
      )[0].innerText = `${apps.taskmgr.memory.toFixed(1)} / 100 GB`;
      $(
        "#win-taskmgr>.main>.cnt.performance>.content>.select-menu>.graph-memory>.right>.data>.value2"
      )[0].innerText = `${apps.taskmgr.memory.toFixed(1)}%`;

      apps.taskmgr.diskSpeed.read =
        apps.taskmgr.disk != 0 ? (Math.random() * 100).toFixed(2) : 0;
      apps.taskmgr.diskSpeed.write =
        apps.taskmgr.disk != 0 ? (Math.random() * 100).toFixed(2) : 0;
      $(
        "#win-taskmgr>.main>.cnt.performance>.content>.performance-graph>.graph-disk>.information>.left>div:nth-child(1)>.value"
      )[0].innerText = `${apps.taskmgr.disk}%`;
      $(
        "#win-taskmgr>.main>.cnt.performance>.content>.performance-graph>.graph-disk>.information>.left>div:nth-child(2)>.value"
      )[0].innerText = `${
        apps.taskmgr.disk != 0 ? Math.random().toFixed(2) : 0
      } 毫秒`;
      $(
        "#win-taskmgr>.main>.cnt.performance>.content>.performance-graph>.graph-disk>.information>.left>div:nth-child(3)>.value"
      )[0].innerText = `${apps.taskmgr.diskSpeed.read} ${
        apps.taskmgr.disk != 0 ? "MB/秒" : "KB/秒"
      }`;
      $(
        "#win-taskmgr>.main>.cnt.performance>.content>.performance-graph>.graph-disk>.information>.left>div:nth-child(4)>.value"
      )[0].innerText = `${apps.taskmgr.diskSpeed.write} ${
        apps.taskmgr.disk != 0 ? "MB/秒" : "KB/秒"
      }`;
      $(
        "#win-taskmgr>.main>.cnt.performance>.content>.select-menu>.graph-disk>.right>.data>.value2"
      )[0].innerText = `${apps.taskmgr.disk}%`;

      $(
        "#win-taskmgr>.main>.cnt.performance>.content>.select-menu>.graph-gpu>.right>.data>.value2"
      )[0].innerText = `${apps.taskmgr.gpu.usage.toFixed(1)}%`;
      ...
      apps.taskmgr.gpu.usage = Number(
        (
          (apps.taskmgr.gpu.d3 +
            apps.taskmgr.gpu.copy +
            apps.taskmgr.gpu.videop +
            apps.taskmgr.gpu.videod) /
          4
        ).toFixed(1)
      );
      ...
      $(
        "#win-taskmgr>.main>.cnt.performance>.content>.performance-graph>.graph-wifi>.information>.left>div:nth-child(1)>.value"
      )[0].innerText = `${apps.taskmgr.wifi[1].toFixed(2)} Mbps`;
      $(
        "#win-taskmgr>.main>.cnt.performance>.content>.performance-graph>.graph-wifi>.information>.left>div:nth-child(2)>.value"
      )[0].innerText = `${apps.taskmgr.wifi[0].toFixed(2)} Mbps`;
      $(
        "#win-taskmgr>.main>.cnt.performance>.content>.select-menu>.graph-wifi>.right>.data>.value2"
      )[0].innerText = `发送: ${apps.taskmgr.wifi[1]} 接收: ${apps.taskmgr.wifi[0]} Mbps`;
    },
    graphLoad: () => {
      $(
        "#win-taskmgr>.main>.cnt.performance>.content>.select-menu>*>.left>.graph-view-cpu"
      )[0].style.backgroundImage = `url("${apps.taskmgr.drawgraph(
        apps.taskmgr.cpuCtx,
        "cpuLastPos",
        apps.taskmgr.cpu
      )}")`;
      ...
      apps.taskmgr.memoryCtx2.fillRect(
        0,
        0,
        (w / 100) * apps.taskmgr.memory,
        h
      );
      $(
        "#win-taskmgr>.main>.cnt.performance>.content>.select-menu>*>.left>.graph-view-disk"
      )[0].style.backgroundImage = `url("${apps.taskmgr.drawgraph(
        apps.taskmgr.diskCtx,
        "diskLastPos",
        apps.taskmgr.disk
      )}")`;
      // 绘制磁盘传输速率
      ...
      apps.taskmgr.drawgraph(
        apps.taskmgr.diskCtx2,
        apps.taskmgr.diskLastPos2.read,
        apps.taskmgr.diskSpeed.read,
        false,
        false
      );
      apps.taskmgr.diskLastPos2.read = [
        w - w / 60,
        (h / 100) * (100 - apps.taskmgr.diskSpeed.read),
      ];
      // 写入速度
      ...
      apps.taskmgr.drawgraph(
        apps.taskmgr.diskCtx2,
        apps.taskmgr.diskLastPos2.write,
        apps.taskmgr.diskSpeed.write,
        false,
        false
      );
      apps.taskmgr.diskLastPos2.write = [
        w - w / 60,
        (h / 100) * (100 - apps.taskmgr.diskSpeed.write),
      ];
      // 绘制网络
      ...
      apps.taskmgr.drawgraph(
        apps.taskmgr.wifiCtx,
        apps.taskmgr.wifiLastPos[0],
        apps.taskmgr.wifi[0],
        false,
        false
      );
      ...
      for (var i = 0; i < 4; i++) {
        ...
        apps.taskmgr.drawgraph(
          apps.taskmgr.gpuCtx[i],
          apps.taskmgr.gpuLastPos[i],
          apps.taskmgr.gpu[["d3", "copy", "videod", "videop"][i]],
          false
        );
        apps.taskmgr.gpuLastPos[i] = [
          w - w / 60,
          (h / 100) *
            (100 - apps.taskmgr.gpu[["d3", "copy", "videod", "videop"][i]]),
        ];
      }
      for (var i = 0; i < 2; i++) {
        ...
        apps.taskmgr.drawgraph(
          apps.taskmgr.gpuCtx2[i],
          apps.taskmgr.gpuLastPos2[i],
          apps.taskmgr.gpuMemory[["private", "public"][i]],
          (setPos = false),
          (move = true),
          (max = [16, 32][i])
        );
    ...
    selectProcess: (elt) => {
      $("#win-taskmgr>.main>.cnt.processes tbody.view>.select").removeClass(
        "select"
      );
      $(elt).addClass("select");
    },
    taskkill: (name) => {
      if (name == "System") {
        ...
        apps.taskmgr.tasks.splice(
          apps.taskmgr.tasks.findIndex((elt) => elt.name == name),
          1
        );
    ...
    initgraph: (prefix, color, setPos = true) => {
      if (setPos == true) {
        apps.taskmgr[prefix + "LastPos"] = [
          apps.taskmgr[prefix + "Canvas"].width,
          apps.taskmgr[prefix + "Canvas"].height,
        ];
      }

      apps.taskmgr[prefix + "Ctx"] =
        apps.taskmgr[prefix + "Canvas"].getContext("2d");
      apps.taskmgr[prefix + "BgCtx"] =
        apps.taskmgr[prefix + "BgCanvas"].getContext("2d");
    ...
    changeGrids: (ctx) => {
      let left = ctx.getImageData(
        0,
        0,
        (ctx.canvas.width / 30) * 29,
        ctx.canvas.height
      );
      let right = ctx.getImageData(
        (ctx.canvas.width / 30) * 29,
        0,
        ctx.canvas.width / 30,
        ctx.canvas.height
  ...
  explorer: {
    init: () => {
      ...
      document.addEventListener("keydown", function (event) {
        if (
          event.key === "Delete" &&
          $(".window.foc")[0].classList[1] == "explorer"
        ) {
    ...
    newtab: (path = "") => {
      ...
      apps.explorer.initHistory(
        apps.explorer.tabs[apps.explorer.tabs.length - 1][0]
      );
      apps.explorer.checkHistory(
        apps.explorer.tabs[apps.explorer.tabs.length - 1][0]
      );
      m_tab.tab("explorer", apps.explorer.tabs.length - 1);
      // }
    ...
    reset: (clear = true) => {
      ...
      )[0].innerHTML = `<style>#win-explorer>.page>.main>.content>.view>.class{margin: 5px 0 0 10px;display: flex;}
    ...
    box-shadow: 0 1px 2px var(--s3d);
      ...
      $("#win-explorer>.path>.tit")[0].innerHTML =
        '<div class="icon" style="background-image: url(\'./apps/icons/explorer/thispc.svg\')"></div><div class="path"><div class="text" onclick="apps.explorer.reset()">此电脑</div><div class="arrow">&gt;</div></div>';
      // if(rename){
      m_tab.rename(
        "explorer",
        '<img src="./apps/icons/explorer/thispc.svg"> 此电脑'
      );
      apps.explorer.tabs[apps.explorer.now][2] = "";
      if (clear) {
        apps.explorer.delHistory(apps.explorer.tabs[apps.explorer.now][0]);
        apps.explorer.pushHistory(
          apps.explorer.tabs[apps.explorer.now][0],
          "此电脑"
        );
      }
      // }
    ...
    select: (path, id) => {
      var elements = document.querySelectorAll(
        "#win-explorer > .main > .content > .view > .select"
      );
    ...
    paste: (path) => {
      ...
      if (pathl[pathl.length - 1] == "C:") {
        ...
        $("#win-explorer>.path>.tit>.icon")[0].style.backgroundImage =
          'url("./apps/icons/explorer/diskwin.svg")';
      } else if (pathl[pathl.length - 1] == "D:") {
        $("#win-explorer>.path>.tit>.icon")[0].style.marginTop = "0px";
        $("#win-explorer>.path>.tit>.icon")[0].style.backgroundImage =
          'url("./apps/icons/explorer/disk.svg")';
      } else {
        $("#win-explorer>.path>.tit>.icon")[0].style.marginTop = "0px";
        $("#win-explorer>.path>.tit>.icon")[0].style.backgroundImage =
          'url("./apps/icons/explorer/folder.svg")';
      }
      pathl.forEach((name) => {
        thisPath += name + "/";
        tmp = tmp["folder"][name];
        $("#win-explorer>.path>.tit>.path")[0].insertAdjacentHTML(
          "beforeend",
          `<div class="text" onmousedown="apps.explorer.goto('${thisPath.substring(
            0,
            thisPath.length - 1
          )}');stop(event);">${name}</div> <div class="arrow">></div> `
        );
      });
      var clipboard = apps.explorer.clipboard;

      if (
        apps.explorer.traverseDirectory(tmp, clipboard[1][0]) ||
        apps.explorer.traverseDirectory(tmp, clipboard[1]["name"])
      ) {
    ...
    del_select: () => {
      ...
      var elements = document.querySelectorAll(
        "#win-explorer>.page>.main>.content>.view>.change"
      );
      ...
      for (var i = 0; i < elements.length; i++) {
        ...
        if (
          inputTag.value == "" ||
          apps.explorer.traverseDirectory(tmp, inputTag.value) ||
          on == inputTag.value
        ) {
          if (
            apps.explorer.traverseDirectory(tmp, inputTag.value) &&
        ...
        if (name_1[0].indexOf("/") > -1)
          alert("恭喜你发现了这个bug,但是太懒了不想修qwq");
        console.log(name_1);
        if (name_1[1] == "txt") {
          icon_ = "icon/files/txt.png";
        } else if (
          name_1[1] == "png" ||
          (name_1[1] == "jpg") | (name_1[1] == "bmp")
        ) {
        ...
        element.parentNode.removeChild(element);
        apps.explorer.goto(
          $("#win-explorer>.path>.tit")[0].dataset.path,
          false
        );
      }
      delete elements;
      apps.explorer.is_use2 = apps.explorer.is_use;
      var elements = document.querySelectorAll(
        "#win-explorer>.page>.main>.content>.view>.select"
      );
      for (var i = 0; i < elements.length; i++) {
        elements[i].classList.remove("select");
      }
    ...
    goto: (path, clear = true) => {
      ...
      $("#win-explorer>.path>.tit>.path")[0].innerHTML =
        '<div class="text" onclick="apps.explorer.reset()">此电脑</div><div class="arrow">&gt;</div>';
      $("#win-explorer>.path>.tit>.icon")[0].style.marginTop = "0px";
      if (pathl[pathl.length - 1] == "C:") {
        $("#win-explorer>.path>.tit>.icon")[0].style.backgroundImage =
          'url("apps/icons/explorer/diskwin.svg")';
        ...
        m_tab.rename(
          "explorer",
          '<img src="./apps/icons/explorer/diskwin.svg" style="margin-top:2.5px">' +
            pathl[pathl.length - 1]
        );
      } else if (pathl[pathl.length - 1] == "D:") {
        $("#win-explorer>.path>.tit>.icon")[0].style.backgroundImage =
          'url("apps/icons/explorer/disk.svg")';
        m_tab.rename(
          "explorer",
          '<img src="./apps/icons/explorer/disk.svg">' + pathl[pathl.length - 1]
        );
      } else {
        $("#win-explorer>.path>.tit>.icon")[0].style.backgroundImage =
          'url("apps/icons/explorer/folder.svg")';
      ...
      pathl.forEach((name) => {
        ...
        )[0].innerHTML += `<div class="text" onclick="apps.explorer.goto('${pathqwq}')">${name}</div><div class="arrow">&gt;</div>`;
        tmp = tmp["folder"][name];
        pathqwq += "/";
      });
      var path_ = path;
    ...
    add: (path, name_, type = "file", command = "", icon = "") => {
      //type为文件类型，只有文件夹files和文件file
      var pathl = path.split("/");
      var icon_ = "";
      let tmp = apps.explorer.path;
      pathl.forEach((name) => {
    ...
    rename: (path) => {
      ...
      let element = document.querySelector(
        "#" + apps.explorer.get_file_id(name)
      );
    ...
    traverseDirectory(dir, name) {
      if (dir["file"] == null && dir["folder"] == null) return false;
      ...
      folder: {
        "C:": {
          folder: {
            "Program Files": {
              folder: {
                WindowsApps: { folder: {}, file: [] },
                Microsoft: { folder: {}, file: [] },
              },
              file: [
                {
                  name: "about.exe",
            ...
            Windows: {
                  ...
                  file: [
                    {
                      name: "calc.exe",
                      ico: "icon/calc.svg",
                      command: "openapp('calc')",
                    },
                    {
                      name: "cmd.exe",
                      ico: "icon/terminal.svg",
                      command: "openapp('terminal')",
                    },
                    {
                      name: "notepad.exe",
                    ...
                    {
                      name: "taskmgr.exe",
                      ico: "icon/taskmgr.png",
                      command: "openapp('taskmgr')",
                    },
                    {
                      name: "winver.exe",
                      ico: "icon/about.svg",
                      command: "openapp('winver')",
                    },
                  ],
                },
              },
              file: [
                {
                  name: "explorer.exe",
                  ico: "icon/explorer.svg",
                  command: "apps.explorer.newtab()",
                },
                {
                  name: "notepad.exe",
                  ico: "icon/notepad.svg",
              ...
              folder: {
                Administrator: {
                  folder: {
                    文档: {
                      folder: {
                        IISExpress: { folder: {}, file: [] },
                        PowerToys: { folder: {}, file: [] },
                      },
                      file: [
                        {
                          name: "瓶盖介绍.doc",
                    ...
                    AppData: {
                      folder: {
                        ...
                        LocalLow: {
                          folder: { Microsoft: { folder: { Windows: {} } } },
                        },
                        Roaming: {
                          folder: {
                            Microsoft: {
    ...
    delHistory: (tab) => {
      apps.explorer.history[tab].splice(
        apps.explorer.historypt[tab] + 1,
        apps.explorer.history[tab].length - 1 - apps.explorer.historypt[tab]
      );
    },
    historyIsEmpty: (tab) => {
    ...
    historyIsFull: (tab) => {
      return (
        apps.explorer.historypt[tab] >= apps.explorer.history[tab].length - 1
      );
    },
    checkHistory: (tab) => {
      ...
      fetch(`https://api.github.com/repos/${repoFullName}`)
        ...
        .then((data) => {
          setTimeout(() => {
            ...
            $("#StarShow").html(
              '<div style="display: flex;"><p>&emsp;&emsp;Star 数量：' +
                starCount +
                ' (实时数据)</p>&emsp;<a class="button" onclick="apps.about.get_star()"><i class="bi bi-arrow-clockwise"></i> 刷新</a></div>'
            );
          }, 200);
        })
        .catch((error) => {
          ...
          $("#StarShow").html(
            '<div style="display: flex;"><p>&emsp;&emsp;哎呀！出错了！</p>&emsp;<a class="button" onclick="apps.about.get_star()"><i class="bi bi-arrow-clockwise"></i> 重试</a></div>'
          );
        });
    },
  },
...
(c) Microsoft Corporation。保留所有权利。
  ...
  edge: {
    ...
    reload: () => {
      if (wifiStatus == false) {
        $("#win-edge>iframe.show").attr(
          "src",
          "./disconnected" + (isDark ? "_dark" : "") + ".html"
        );
      } else {
        $("#win-edge>iframe.show").attr(
          "src",
    ...
    goto: (u, clear = true) => {
      if (wifiStatus == false) {
        ...
        if (
          ...
          !u.match(/^mainpage.html$/)
        ) {
          // 启用必应搜索
          $("#win-edge>iframe.show").attr(
            "src",
            "https://bing.com/search?q=" + encodeURIComponent(u)
          );
          m_tab.rename("edge", u);
        }
        // 检测网址是否带有http头
    ...
    delHistory: (tab) => {
      apps.edge.history[tab].splice(
        apps.edge.historypt[tab] + 1,
        apps.edge.history[tab].length - 1 - apps.edge.historypt[tab]
      );
    },
    historyIsEmpty: (tab) => {
...
let widgets = {
  widgets: {
    add: (arg) => {
      if (
        $(`.wg.${arg}.menu,.wg.${arg}.toolbar,.wg.${arg}.desktop`).length != 0
      ) {
        return;
      }
  ...
  calc: {
    init: () => {
      widgetCalculator = new Calculator(
        $(
          "*:not(.template)>*>.wg.calc>.content>.container>#calc-input-widgets"
        )[0],
        $("*:not(.template)>*>.wg.calc>.content")[0]
      );
    },
    remove: () => {
  ...
  weather: {
    ...
    update: () => {
      ...
      $.getJSON(
        "https://assets.msn.cn/service/weatherfalcon/weather/overview?locale=zh-cn&ocid=msftweather"
      ).then((r) => {
        let inf = r.value[0].responses[0].weather[0].current;
        // console.log(inf.icon,wic[inf.icon]);
        $(".wg.weather>.content>.img").attr(
          "src",
          `https://assets.msn.cn/weathermapdata/1/static/weather/Icons/taskbar_v10/Condition_Card/${
            wic[inf.icon]
          }.svg`
        );
        $(".wg.weather>.content>.text>.temperature").text(`${inf.temp}℃`);
        $(".wg.weather>.content>.text>.detail").text(
          `${inf.cap} 体感温度${inf.feels}℃`
        );
      });
    },
  },
  monitor: {
    ...
    init: () => {
      if (
        $("*:not(.template)>*>.wg.monitor")[0].classList.contains("toolbar")
      ) {
        $("*:not(.template)>*>.wg.monitor>.content>.container>svg>circle").attr(
          "r",
          "15px"
        );
      } else {
    ...
    update: () => {
      $(
        "*:not(.template)>*>.wg.monitor>.content>.container>svg>circle:last-child"
      ).css(
        "stroke-dasharray",
        `${
          widgets.monitor.type != "gpu"
            ? widgets.monitor.type.match("wifi")
              ? widgets.monitor.type == "wifi-send"
                ? (apps.taskmgr.wifi[1] / 100) *
                  (Math.PI *
                    $(
                      "*:not(.template)>*>.wg.monitor>.content>.container>svg>circle:last-child"
                    )[0].r.baseVal.value *
                    2)
                : (apps.taskmgr.wifi[0] / 100) *
                  (Math.PI *
                    $(
                      "*:not(.template)>*>.wg.monitor>.content>.container>svg>circle:last-child"
                    )[0].r.baseVal.value *
                    2)
              : (apps.taskmgr[widgets.monitor.type] / 100) *
                (Math.PI *
                  $(
                    "*:not(.template)>*>.wg.monitor>.content>.container>svg>circle:last-child"
                  )[0].r.baseVal.value *
                  2)
            : (apps.taskmgr.gpu.usage / 100) *
              (Math.PI *
                $(
                  "*:not(.template)>*>.wg.monitor>.content>.container>svg>circle:last-child"
                )[0].r.baseVal.value *
      ...
      if (widgets.monitor.type == "cpu" || widgets.monitor.type == "gpu") {
        ...
        $("*:not(.template)>*>.wg.monitor>.content>.text>.type")[0].innerText =
          widgets.monitor.type == "cpu" ? "CPU利用率" : "GPU利用率";
      ...
      $("*:not(.template)>*>.wg.monitor>.content>.text>.value")[0].innerText =
        (widgets.monitor.type != "gpu"
          ? widgets.monitor.type.match("wifi")
            ? widgets.monitor.type == "wifi-send"
              ? apps.taskmgr.wifi[1]
          ...
          : apps.taskmgr.gpu.usage
        ).toFixed(widgets.monitor.type.match("wifi") ? 2 : 1) +
...
function widgetsMove(elt, e) {
  if (elt.classList.contains("desktop") && edit_mode == true) {
    ...
    let gridrow = window
      ...
      .gridRowEnd.replace("span ", "");
    let gridcol = window
      .getComputedStyle(elt, null)
      .gridColumnEnd.replace("span ", "");
    let gridrowmax = window
      .getComputedStyle($("#desktop-widgets")[0], null)
      .gridTemplateRows.split(" ").length;
    let gridcolmax = window
      .getComputedStyle($("#desktop-widgets")[0], null)
    ...
    function widgetsMoving(e) {
      ...
      gridnow = {
        col: (
          (width / 2 + elt.getBoundingClientRect().right - 20) /
            ((gridcolmax * 83 + 10 * (gridcolmax - 1)) / gridcolmax) -
          gridcol +
          (gridcol - 2) * 0.5
        ).toFixed(0),
        row: (
          (height / 2 + top - 20) /
            ((gridrowmax * 83 + 10 * (gridrowmax - 1)) / gridrowmax) +
          (2 - gridrow) * 0.5
        ).toFixed(0),
      };
      gridnow.col =
        gridnow.col <= Math.floor(gridcol / 2)
          ? 1 + Math.floor(gridcol / 2)
          : gridnow.col >
            gridcolmax -
              gridcol +
              (gridcol % 2 ? (Number(gridcol) + 1) / 2 : gridcol / 2)
          ? gridcolmax -
            gridcol +
            (gridcol % 2 ? (Number(gridcol) + 1) / 2 : gridcol / 2)
          : gridnow.col;
      ...
      $("#desktop-widgets>.widgets-move").css(
        "cssText",
        `grid-column: ${
          gridcolmax - gridnow.col
        } / span ${gridcol}; grid-row: ${gridnow.row} / span ${gridrow}`
      );
    }
    function up() {
...
function msgDoneOperate(){
    ...
    // 延迟0.1s以使enter键的blur事件不影响focus事件
    setTimeout(() => {
        $("#copilot>.inputbox>.input").focus();
    }, 100);
}
let isFirstChat=true;  // 标识是否是第一次聊天
let copilot = {
  history: [],
  init: () => {
    $("#copilot>.chat").html("");
    copilot.send(
      `请使用中文对话，请使用中文对话，请使用中文对话！。你一个是ai助手，名叫AI Copilot，是由github@NB-Group开发的。
    ...
    $("#copilot>.chat").append(
      `<div class="line system"><p class="text" id="init-message">正在初始化...</p></div>`
    );
  ...
  send: (t, showusr = true, role = "user") => {      
    ...
    if (t.length == 0) {
      $("#copilot>.chat").append(
        `<div class="line system"><p class="text">系统表示请发一些有意义的东西</p></div>`
      );
      $("#copilot>.chat").scrollTop($("#copilot>.chat")[0].scrollHeight);
      msgDoneOperate();
      return;
    ...
    if (showusr)
      $("#copilot>.chat").append(
        `<div class="line user"><p class="text">${t}</p></div>`
      );
    ...
    $.post({
      ...
      .then((rt) => {
        ...
        if (rt == "请求过于频繁，等待10秒再试...") {
          $("#copilot>.chat").append(
            `<div class="line system"><p class="text">api繁忙，过一会儿再试(实在不行刷新重新开始对话)</p></div>`
          );
          $("#copilot>.chat").scrollTop($("#copilot>.chat")[0].scrollHeight);
          msgDoneOperate();
          return;
        ...
        if (r.length) {
          for (const i of r) {
            if (/{openapp .+?}/.test(i)) {
              ...
              rt = rt.replace(
                i,
                `<div class="action"><p class="tit">打开应用</p><p class="detail">${$(
                  `.window.${t}>.titbar>p`
                ).text()}</p></div>`
              );
              ...
              rt = rt.replace(
                i,
                `<div class="action"><p class="tit">打开URL</p><p class="detail">${decodeHtml(
                  t
                )}</p></div>`
              );
              ...
              rt = rt.replace(
                i,
                `<div class="action"><p class="tit">反馈</p><p class="detail">关于 Windows 12 网页版</p></div>`
              );
            } else if (/{feedback copilot}/.test(i)) {
              shownotice("feedback-copilot");
              rt = rt.replace(
                i,
                `<div class="action"><p class="tit">反馈</p><p class="detail">关于 Windows 12 Copilot</p></div>`
              );
            } else if (/{settheme .+?}/.test(i)) {
              let t = i.match(/(?<={settheme ).+(?=})/)[0];
              if (
                (t == "light" && $(":root").hasClass("dark")) ||
                (t == "dark" && !$(":root").hasClass("dark"))
              )
                toggletheme();
              rt = rt.replace(
                i,
                `<div class="action"><p class="tit">切换外观模式</p><p class="detail">${
                  t == "dark" ? "深色" : "浅色"
                } 模式</p></div>`
              );
            }
          ...
          $("#copilot>.chat").append(
            `<div class="line ai"><div class="text">${rt}</div></div>`
          );
        } else {
          $("#copilot>.chat").append(
            `<div class="line ai"><p class="text">${decodeHtml(rt)}</p></div>`
          );
        }
      ...
      .fail((r) => {
        console.log(r);
        $("#copilot>.chat").append(
          `<div class="line system"><p class="text">发生错误，请查看控制台输出或重试</p></div>`
        );
        $("#copilot>.chat").scrollTop($("#copilot>.chat")[0].scrollHeight);
        msgDoneOperate()
      });
...
setTimeout(
  ...
  1000 - da.getMilliseconds()
); //修复时间不精准的问题。以前的误差：0-999毫秒；现在：几乎没有
let d = new Date();
...
function openapp(name) {
  if (
    taskmgrTasks.findIndex((elt) => elt.link == name) > -1 &&
    apps.taskmgr.tasks.findIndex((elt) => elt.link == name) == -1
  ) {
    apps.taskmgr.tasks.splice(
      apps.taskmgr.tasks.length,
      0,
      taskmgrTasks.find((elt) => elt.link == name)
    );
  }
...
function maxwin(name, trigger = true) {
  if ($(".window." + name).hasClass("max")) {
    ...
    if (
      ...
      $(".window." + name).css(
        "left",
        `${$(".window." + name).attr("data-pos-x")}`
      );
      $(".window." + name).css(
        "top",
        `${$(".window." + name).attr("data-pos-y")}`
      );
    }
    // }
  } else {
  ...
  for (let i = 0; i < wo.length; i++) {
    ...
    if (topmost.includes(wo[wo.length - i - 1])) {
      win.css(
        ...
        10 +
          i +
          50 /*这里的50可以改，不要太大，不然会覆盖任务栏；不要太小，不然就和普通窗口没有什么区别了。随着版本的更新，肯定会有更多窗口，以后就可以把数字改打一点点*/
      );
    } else {
  ...
  if (this.classList.contains("active")) {
    ...
    if (name == "fly") {
      ...
      if (localStorage.getItem(FLY_HIDDEN_LIST_KEY)) {
        const flyHiddenData = JSON.parse(
          localStorage.getItem(FLY_HIDDEN_LIST_KEY)
        );
...
function saveDesktop() {
  localStorage.setItem(
    "desktop",
    /*$('#desktop')[0].innerHTML*/ JSON.stringify(desktopItem)
  );
...
function setIcon() {
  if (Array.isArray(JSON.parse(localStorage.getItem("desktop")))) {
    $(
      "#desktop"
    )[0].innerHTML = `<div ondblclick="openapp('explorer');" ontouchstart="openapp('explorer');" oncontextmenu="return showcm(event,'desktop.icon',['explorer',-1]);" appname="explorer">
  ...
  if (Array.isArray(JSON.parse(localStorage.getItem("sys_setting")))) {
    ...
    if (/^(1|0)+$/.test(sys_setting_back.join("")) /* 只含有0和1 */) {
      ...
      for (var i = 0; i < sys_setting.length; i++) {
        document
          .getElementById("sys_setting_" + (i + 1))
          ?.setAttribute(
            "class",
            "a checkbox" + (sys_setting[i] ? " checked" : "")
          ); //设置class属性
        if (i == 5) {
  ...
  if (localStorage.getItem("root_class")) {
    $(":root")[0].className =
      localStorage.getItem("root_class") + " " + (isDark ? "dark" : "");
  }
}
...
document.getElementsByTagName("body")[0].onload = function nupd() {
  ...
  document.querySelectorAll(".window>div.resize-bar").forEach((w) => {
    for (const n of [
      ...
      w.insertAdjacentHTML(
        "afterbegin",
        `<div class="resize-knob ${n}" onmousedown="resizewin(this.parentElement.parentElement, '${n}', this)"></div>`
      );
    }
  });
  ...
  document
    .querySelector(".rainbow-container-main")
    .setAttribute(
      "style",
      "display:" + (use_mic_voice ? "block" : "none") + ";"
    );
  // loadlang();
};
